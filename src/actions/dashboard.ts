...
export async function getDashboardData() {
  ...
  // Quét & tạo cảnh báo công nợ quá hạn / công trình trễ tiến độ (không có cron, chạy khi vào Dashboard).
  // after() chạy sau khi response đã trả về cho client — không chặn render, nhưng vẫn đảm bảo hoàn tất.
  ...
  // Mỗi nhóm số liệu độc lập với nhau — mỗi nhóm trả về 1 object riêng (không ghi đè biến
  // chung) rồi gộp lại sau khi TẤT CẢ đã chạy xong song song qua Promise.all. Trước đây mỗi
  // nhóm await tuần tự nối tiếp nhau (~20 round-trip nối tiếp tới Supabase), giờ chạy đồng thời.

  const crmStats = async () => {
    if (!(isAdmin || isManager || isSales)) return {};
  ...
  const financeStats = async () => {
    if (!(isAdmin || isManager || isAccountant)) return { stats: {}, recentReceipts: [] as any[] };
  ...
  // Lợi nhuận tính từ dữ liệu thật (phiếu thu - vật tư xuất kho - lương thợ - chi phí khác),
  // KHÔNG dùng cột `estimatedProfit` gõ tay trên form công trình (đó chỉ là dự toán).
  const profitStats = async () => {
  ...
  const inventoryStats = async () => {
    if (!(isAdmin || isManager || user.roles.includes(ROLES.WAREHOUSE_STAFF))) return {};
  ...
  const operationsStats = async () => {
    if (!(isAdmin || isManager || isConstruction || isProduction)) return {};
  ...
  const recentActivitiesQuery = async () => {
    if (!(isAdmin || isManager)) return [] as any[];
  ...
  const [crm, finance, profit, inventory, operations, recentActivities] = await Promise.all([
    crmStats(),
    financeStats(),
    profitStats(),
    inventoryStats(),
  ...
  return {
    ...
    recentReceipts: finance.recentReceipts,
  };
}
